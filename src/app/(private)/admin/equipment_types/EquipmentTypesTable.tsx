...
export function EquipmentTypesTable({ data, onCreate, onUpdate, onToggle, onDelete }: EquipmentTypesTableProps) {
  ...
  const [rows, setRows] = useState<EquipmentTypeRow[]>(data);
  ...
  const submitForm = async (values: EquipmentTypeFormValues) => {
    ...
    try {
      if (dialogState?.mode === "edit" && dialogState.equipmentType) {
        setRows((current) =>
          current.map((item) => (String(item.id) === String(dialogState.equipmentType.id) ? { ...item, ...values } : item))
        );
  ...
  const handleDelete = async () => {
    ...
    setRows((current) => current.filter((item) => String(item.id) !== String(confirmEquipmentType.id)));
    try {
  ...
  const handleToggle = async (equipmentType: EquipmentTypeRow) => {
    ...
    setRows((current) =>
      current.map((item) => (String(item.id) === String(equipmentType.id) ? { ...item, status: nextStatus } : item))
    );
        ...
        <Table aria-busy={Boolean(pendingAction) || isRefreshing} className="[&_tbody_tr:hover]:bg-gray-50/50">
          <TableHeader>
            <TableRow className="bg-gray-50 text-[11px] uppercase tracking-[0.2em] text-gray-500">
              ...
              >
                Descrição {sortBy === "description" ? (sortDir === "asc" ? "▲" : "▼") : null}
              </TableHead>
              <TableHead
                className="cursor-pointer"
                onClick={() => {
          ...
          <TableBody>
            {visibleEquipmentTypes.map((equipmentType) => (
              <TableRow key={String(equipmentType.id)} className="border-gray-100">
                <TableCell className="text-center font-semibold text-gray-800">{equipmentType.id ?? "—"}</TableCell>
                ...
                <TableCell className="text-gray-800">{equipmentType.name}</TableCell>
